  ...
  if (!allSongs?.length && search && !loading) {
    return (
      <div className=" flex flex-col justify-center items-center text-white mt-5">
        <p className="font-semibold text-lg">Couldn't find "{search}"</p>
    ...
    <div className="max-h-[75vh] overflow-y-scroll mt-6 pb-32 lg:pb-0" >
      {allSongs?.map((item) => (
        <motion.div
          ...
          key={item._id}
          className={`text-white mt-1 flex justify-between items-center py-4 px-4 hover:bg-gray-700 cursor-pointer rounded-[8px] ${currentSong._id === item._id ? "bg-gray-700" : ""
            }`}
        ...
        >
          <div className="flex items-center gap-4 ">
            ...
            />
            <div>
              <h4 className="lg:text-[18px] text-base  line-clamp-1">{item.title}</h4>
              <p className="opacity-60 lg:text-[14px] text-xs ">{item.artist}</p>
            </div>
          </div>
